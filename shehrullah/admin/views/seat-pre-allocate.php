...
function _handle_post()
{
    ...
    if ($action === 'search') {
        ...
        if (empty($its_id) || empty($hof_id) || empty($area_code) || empty($seat_number)) {
            setSessionData(TRANSIT_DATA, 'Please fill all required fields including seat number.');
            return;
        }
        ...
        if ($result['success']) {
            ...
            if ($error === 'GENDER_MISMATCH') {
                ...
                setSessionData(TRANSIT_DATA, 'Failed to pre-allocate seat. Please try again. Error: ' . $error);
            }
        }
