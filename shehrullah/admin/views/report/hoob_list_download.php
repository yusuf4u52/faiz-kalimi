...
$hijri_year = get_current_hijri_year();
    ...
    echo "S/No,ITSNo,Hoobname,ReceiptNo,ReceiptDate,year,ReceiptName,Amount,PayType,ChequeNo,Remarks" . PHP_EOL;
    $sno = 0;
    foreach($report_data as $row) {
        $sno++;
        echo "$sno,{$row->ITSNo},{$row->Hoobname},{$row->ReceiptNo},{$row->ReceiptDate},{$row->year},{$row->ReceiptName},{$row->Amount},{$row->PayType},{$row->ChequeNo},{$row->Remarks}" . PHP_EOL;
    }