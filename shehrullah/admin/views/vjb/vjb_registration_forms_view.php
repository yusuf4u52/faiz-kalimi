...
if (!defined('VJB_PRINT_LIB_ONLY')) {
    define('VJB_PRINT_LIB_ONLY', true);
}
include_once __DIR__ . '/../../../views/vjb/vjb_print.php';
?>
<style>
.vjb-forms-wrapper {
    font-size: 14px;
    ...
    <?php foreach ($records as $row): ?>
        ...
        <div class="vjb-form-page">
            <table class="vjb-table">
                ...
                <tr>
                    <td colspan="3" class="vjb-band-blue vjb-year"><?= h($vajebaat_prev_label) ?></td>
                ...
                <tr>
                    <td colspan="3" class="vjb-band-cyan vjb-year"><?= h($niyaz_prev_label) ?></td>
                ...
                <tr>
                    <td colspan="3" class="vjb-band-blue vjb-year"><?= h($ikram_prev_label) ?></td>
        ...
        $vjbFormError = '';
        try {
            $vjbFormImage = vjb_generate_form_jpeg_base64([
                'itsid' => $row->ITS_ID ?? '',
                'name' => $row->Full_Name ?? '',
                'address' => $row->Address ?? '',
                'mobile' => $row->Mobile ?? '',
                'last_vajebaat2' => $vajebaatPrev,
                'form_mardo_count' => (int)($row->gents_count ?? 0),
                'form_bairo_count' => (int)($row->ladies_count ?? 0),
                'form_kids_count' => (int)($row->kids_count ?? 0),
                'form_amwat_count' => (int)($row->amwat_count ?? 0),
                'form_hamal_count' => (int)($row->hamal_count ?? 0),
            ]);
        } catch (Throwable $e) {
            $vjbFormImage = '';
            $vjbFormError = $e->getMessage();
        }
        ?>
        <div class="vjb-form-page">
            <?php if ($vjbFormImage !== ''): ?>
                <img class="vjb-image-page" src="data:image/jpeg;base64,<?= $vjbFormImage ?>" alt="Vajebaat Form">
            <?php else: ?>
                <div class="text-center" style="padding-top: 40mm;">
                    Unable to render image-based Vajebaat form.
                    <?php if ($vjbFormError !== ''): ?>
                        <div style="margin-top: 8px; font-size: 12px; color: #b00020;"><?= h($vjbFormError) ?></div>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        </div>
