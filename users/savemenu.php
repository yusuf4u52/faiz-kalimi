...
if( isset($_POST['action']) && $_POST['action'] == 'add_menu' ) {
    $result = mysqli_query($link, "SELECT * FROM menu_list WHERE `menu_date` = '".$_POST['menu_date']."'") or die(mysqli_error($link));
    if($result->num_rows > 0) {
        $sql = "UPDATE menu_list SET `menu_type` = '" . $_POST['menu_type'] . "', `menu_item` = '".serialize($menu_item)."' WHERE `menu_date` = '".$_POST['menu_date']."'";
    } else {
        $sql = "INSERT INTO menu_list (`menu_date`,`menu_type`,`menu_item`) VALUES ('" . $_POST['menu_date'] . "', '" . $_POST['menu_type'] . "', '" . serialize($menu_item) . "')";
    }
    mysqli_query($link, $sql) or die(mysqli_error($link));
    header("Location: /fmb/users/menu_list.php");
}
