			...
			<div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
				...
				?>
						<div class="panel panel-default">
							...
							<div id="collapse<?php echo $musaid['id']; ?>" class="panel-collapse collapse <?php if (mysqli_num_rows($musaid_list) == 1) echo "in"; ?>" role="tabpanel" aria-labelledby="heading<?php echo $musaid['id']; ?>">
									...
									<table class="table table-hover">
										...
										<tbody>
											...
											while ($values = mysqli_fetch_assoc($result)) {
												$commit = mysqli_query($link, "SELECT concat(commit_date, ' / ', rs) as data FROM hub_commitment where rs !=0 and thali='" . $values['Thali'] . "'");
												$all_dates = array();
												while ($all_data = mysqli_fetch_assoc($commit)) {
													$all_dates[] = $all_data['data'];
												}

												$comments = mysqli_query($link, "SELECT `hub_commitment`.`comments`, `hub_commitment`.`timestamp`, `thalilist`.`Email_ID` FROM hub_commitment INNER JOIN `thalilist` on `hub_commitment`.`author_id` = `thalilist`.`id` where comments is not null and `hub_commitment`.`thali`='" . $values['Thali'] . "' ORDER BY `timestamp` DESC");
											?>
												<form method="post">
													<tr>
