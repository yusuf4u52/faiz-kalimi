...
$menu_item = mysqli_query($link, "SELECT `menu_item` FROM menu_list WHERE `menu_date` = '" . $tomorrow_date . "' AND `menu_type` = 'thaali'");
if ($menu_item->num_rows > 0) {
	...
	$smailSent = sendEmail($skipemails, 'No Thaali Update ' . $tomorrow_date, $skipmsg, null, null, true);
	if ($smailSent) { 
		echo 'Skip Email';
	}
}
