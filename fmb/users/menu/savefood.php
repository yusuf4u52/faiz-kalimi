...
if ($action === 'add_food') {
    ...
    if ($dishName === '' || !in_array($dishType, VALID_DISH_TYPES, true)) {
        header("Location: /fmb/users/menu/food.php?action=error");
        exit;
    }
    ...
    try {
        ...
        if (mysqli_errno($link) === 1062) { // ER_DUP_ENTRY
            header("Location: /fmb/users/menu/food.php?action=duplicate&dish=" . urlencode($dishName));
            exit;
        }
        throw $e;
    }
    header("Location: /fmb/users/menu/food.php?action=add&dish=" . urlencode($dishName));
    exit;
}
...
if ($action === 'edit_food') {
    ...
    header("Location: /fmb/users/menu/food.php?action=edit&dish=" . urlencode($dishName));
    exit;
}
...
if ($action === 'delete_food') {
    ...
    header("Location: /fmb/users/menu/food.php?action=delete&dish=" . urlencode($dishName));
    exit;
}

// No recognised action — send the user back rather than rendering a blank page.
header("Location: /fmb/users/menu/food.php");
exit;
