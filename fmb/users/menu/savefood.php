...
if( isset($_POST['action']) && $_POST['action'] == 'add_food' ) {
    ...
    header("Location: /fmb/users/menu/food.php?action=add&dish=".$_POST['dish_name']);
}
...
if( isset($_POST['action']) && $_POST['action'] == 'edit_food' ) {
    ...
    mysqli_query($link,$sql) or die(mysqli_error($link));
    header("Location: /fmb/users/menu/food.php?action=edit&dish=".$_POST['dish_name']);
}

if( isset($_POST['action']) && $_POST['action'] == 'delete_food' ) {
    $sql = "DELETE FROM food_list WHERE `id` = '".$_POST['food_id']."'";
    mysqli_query($link,$sql) or die(mysqli_error($link));
    header("Location: /fmb/users/menu/food.php?action=delete&dish=".$_POST['dish_name']);
}
?>
