...
if (isset($_POST)) {
  ...
  if (isset($_POST['action']) && $_POST['action'] == 'change_musaid' && isset($_POST['musaid'])) {
    ...
    $change_musaid_query = "UPDATE `thalilist` SET `musaid` = '$clean_musaid' WHERE id = '" . $_POST['id'] . "'";
...
if (isset($_GET['year'])) {
  ...
  $query = "SELECT id, Thali, tiffinno, NAME, CONTACT, WhatsApp, Active, Transporter, thalisize, wingflat, society, extraRoti, yearly_hub, ITS_No, Email_ID, SEmail_ID, thalicount, Thali_start_date, Thali_stop_date, Full_Address, musaid, Paid, (Previous_Due + yearly_hub + Zabihat - Paid) AS Total_Pending FROM thalilist";
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <form method="POST" autocomplete="off">
        ...
        <div class="modal-body">
          <input type="email" class="form-control" id="Email_ID" placeholder="Email" name="Email_ID" value='<?php echo $values['Email_ID'] ?>' pattern="[a-z0-9._%+\-]+@gmail.com$" required>
