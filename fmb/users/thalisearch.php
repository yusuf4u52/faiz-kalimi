...
if (isset($_GET['year'])) {
    ...
    // BUG FIX: this used to hardcode "FROM thalilist", so picking a past
    // Hijri year in the UI computed $thalilist_tablename above but never
    // actually queried it — every search silently ran against the
    // current-year table regardless of which year was selected. Table
    // names can't be bound as prepared-statement params, but
    // $thalilist_tablename is safe to interpolate here: it was validated
    // a few lines up against a strict `1[3-5][0-9]{2}` whitelist (or set
    // to the literal 'thalilist'), so this can never be attacker-controlled.
    $baseQuery = "SELECT id, Thali, tiffinno, NAME, CONTACT, WhatsApp, Active, Transporter, thalisize, wingflat, sabeelType, sector, society, extraRoti, lessRice, previous_hub, yearly_hub, Zabihat, ITS_No, Email_ID, SEmail_ID, thalicount, Thali_start_date, Thali_stop_date, Full_Address, musaid, Paid, hardstop, (Previous_Due + yearly_hub - Paid) AS Total_Pending FROM `$thalilist_tablename`";
