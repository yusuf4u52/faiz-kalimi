...
if( isset($_POST['action']) && $_POST['action'] == 'add_tcount' ) {
    ...
    if ($atcount->num_rows > 0) {
        ...
        $sql = "UPDATE  transporter_thali_count SET `thali_count` = '".$_POST['thali_count']."', `comment` = '".$_POST['comment']."', `counted_by` = '" . $_POST['counted_by'] . "' WHERE `id` = '".$row_atcount['id']."'";
        ...
        $sql = "INSERT INTO  transporter_thali_count (`transporter_id`, `count_date`, `thali_count`, `comment`, `counted_by`) VALUES ('" . $_POST['transporter_id'] . "', '" . $_POST['count_date'] . "', '" . $_POST['thali_count'] . "', '" . $_POST['comment'] . "', '" . $_POST['counted_by'] . "')"; 
        mysqli_query($link, $sql) or die(mysqli_error($link));
...
if( isset($_POST['action']) && $_POST['action'] == 'edit_tcount' ) {
    $sql = "UPDATE  transporter_thali_count SET `transporter_id` = '".$_POST['transporter_id']."', `count_date` = '".$_POST['count_date']."', `thali_count` = '".$_POST['thali_count']."', `comment` = '".$_POST['comment']."', `counted_by` = '" . $_POST['counted_by'] . "' WHERE `id` = '".$_POST['tcount_id']."'";
    mysqli_query($link,$sql) or die(mysqli_error($link));
    header("Location: /fmb/users/transporterthalicount.php?action=edit&transporter=".$_POST['transporter_id']."&count_date=".$_POST['count_date']);
}
