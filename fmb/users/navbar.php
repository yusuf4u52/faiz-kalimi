            ...
            <div class="col-8 text-end">
                ...
                <?php if (!empty($values['yearly_hub'])) {
                    if ($values['Active'] == 1) { 
						...
						SELECT `thali`, MIN(`stop_date`) AS start_date, MAX(`stop_date`) AS end_date, DATE_ADD(MAX(`stop_date`), INTERVAL 1 DAY) AS next_active_date FROM grouped_dates GROUP BY `thali`, group_key ORDER BY end_date DESC LIMIT 1;") or die(mysqli_error($link));
