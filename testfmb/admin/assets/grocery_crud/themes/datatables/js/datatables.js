var default_per_page =
  typeof default_per_page !== "undefined" ? default_per_page : 25;
var oTable = null;
var oTableArray = [];
var oTableMapping = [];
...
$(document).ready(function () {
  ...
  $(".groceryCrudTable tfoot input").keyup(function () {
    chosen_table = datatables_get_chosen_table(
      $(this).closest(".groceryCrudTable")
    );

    chosen_table.fnFilter(
      this.value,
      chosen_table.find("tfoot input").index(this) + 1
    );
    ...
    if (use_storage) {
      ...
      localStorage.setItem(
        "datatables_search_" + unique_hash,
        '["' + search_values_array.join('","') + '"]'
      );
    }
  });
  ...
  if (search_values !== null) {
    $.each($.parseJSON(search_values), function (num, val) {
      if (val !== "") {
        $(".groceryCrudTable tfoot tr th:eq(" + num + ")")
          .children(":first")
          .val(val);
      }
    });
...
function loadDataTable(this_datatables) {
  return $(this_datatables).dataTable({
    ...
    oTableTools: {
      ...
      sSwfPath:
        base_url +
        "assets/grocery_crud/themes/datatables/extras/TableTools/media/swf/copy_csv_xls_pdf.swf",
    },
  });
}
  ...
  if (confirm(message_alert_delete)) {
    $.ajax({
      ...
      success: function (data) {
        if (data.success) {
          chosen_table = datatables_get_chosen_table(
            $("tr#row-" + row_id).closest(".groceryCrudTable")
          );
