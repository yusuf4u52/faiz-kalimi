...
if ($action === 'change_menu') {
    header("Location: /testfmb/users/index.php?action=" . urlencode($resultAction) . "&date=" . urlencode($menu_date));
    exit;
}
...
if ($action === 'feedback_menu') {
    ...
    header("Location: /testfmb/users/index.php?action=" . urlencode($feedbackAction) . "&date=" . urlencode($menu_date));
    exit;
}
