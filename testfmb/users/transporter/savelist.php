...
if ($action === 'add_transporter') {
    ...
    if ($name === '' || !is_valid_mobile($mobile) || !is_valid_gmail_address($email)) {
        header("Location: /testfmb/users/transporter/list.php?action=error");
        exit;
    }
    ...
    try {
        ...
        if (mysqli_errno($link) === 1062) { // ER_DUP_ENTRY
            header("Location: /testfmb/users/transporter/list.php?action=duplicate&Name=" . urlencode($name));
            exit;
        }
        throw $e;
    }
    header("Location: /testfmb/users/transporter/list.php?action=add&Name=" . urlencode($name));
    exit;
}
...
if ($action === 'edit_transporter') {
    ...
    header("Location: /testfmb/users/transporter/list.php?action=edit&Name=" . urlencode($name));
    exit;
}
...
if ($action === 'delete_transporter') {
    ...
    header("Location: /testfmb/users/transporter/list.php?action=delete&Name=" . urlencode($name));
    exit;
}

// No recognised action — send the user back rather than rendering a blank page.
header("Location: /testfmb/users/transporter/list.php");
exit;
