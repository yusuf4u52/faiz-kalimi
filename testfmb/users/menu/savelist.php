...
if ($menuType !== null && !in_array($menuType, VALID_MENU_TYPES, true)) {
    header("Location: /testfmb/users/menu/list.php?action=error");
    exit;
}
...
if ($action === 'add_menu') {
    ...
    try {
        ...
        if (mysqli_errno($link) === 1062) { // ER_DUP_ENTRY
            header("Location: /testfmb/users/menu/list.php?action=existed&date=" . urlencode($menuDate));
            exit;
        }
        throw $e;
    }
    ...
    header("Location: /testfmb/users/menu/list.php?action=add&date=" . urlencode($menuDate));
    exit;
}
...
if ($action === 'edit_menu') {
    ...
    handleLessRiceUsers($link, $menu_item, $menuDate);
    header("Location: /testfmb/users/menu/list.php?action=edit&date=" . urlencode($menuDate));
    exit;
}
...
if ($action === 'delete_menu') {
    ...
    db_query($link, "DELETE FROM user_menu WHERE `menu_date` = ?", "s", [$menuDate]);
    header("Location: /testfmb/users/menu/list.php?action=delete&date=" . urlencode($menuDate));
    exit;
}

// No recognised action.
header("Location: /testfmb/users/menu/list.php");
exit;
